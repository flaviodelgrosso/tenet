...
impl AcpLaunch {
  ...
  fn custom(custom: loops_domain::config::CustomAgentConfig) -> Self {
    Self {
      registry: None,
      custom: Some(custom),
    }
...
async fn list_registry_agents(cwd: &Path, query: Option<&str>) -> Result<()> {
  ...
  if matched == 0 {
    if let Some(query) = query {
      ...
      println!("Next: run `loops agents list` or choose an ID from a refreshed Registry.");
    } else {
...
async fn select_registry_agent(cwd: &Path, id: &str) -> Result<()> {
  let id = id.trim();
  if id.is_empty() {
    anyhow::bail!("Registry agent ID cannot be empty; next action: run `loops agents list`");
  }

  let index = load_registry_index(cwd).await?;
  let selected = index.agents.iter().find(|agent| agent.id == id).ok_or_else(|| {
    anyhow::anyhow!("unknown Registry agent {id:?}; next action: run `loops agents list` or `loops agents search <query>`")
  })?;

  let path = loops_domain::config::config_path(cwd);
  if !path.exists() {
    loops_domain::config::ensure_config(cwd).await?;
  }
  ...
  let agent = document
    ...
    .ok_or_else(|| anyhow::anyhow!("invalid loops.toml: [agent] must be a table"))?;
  ...
  let configured_id = match agent.get("id") {
    ...
    Some(_) => anyhow::bail!("invalid loops.toml: agent.id must be a non-empty string"),
  };
  ...
  let body = toml::to_string_pretty(&document).context("serialize loops.toml")?;

  tokio::fs::write(
    &path,
    format!(
      "#:schema {}\n\n{body}",
      loops_domain::config::CONFIG_SCHEMA_URL
    ),
  )
  .await
  ...
  println!("Next: run `loops agents doctor` to check launch readiness.");
  Ok(())
}
...
async fn setup_registry_agent(cwd: &Path, requested_id: Option<&str>, yes: bool) -> Result<()> {
  ...
  if !yes {
    ...
    println!("Confirmation required: run `loops agents setup {id} --yes` to install its binary distribution.");
    return Ok(());
  }
...
async fn selected_registry_id(cwd: &Path) -> Result<String> {
  let config = loops_domain::config::read_config(cwd).await?;
  match (config.agent.id.as_deref().filter(|id| !id.trim().is_empty()), config.agent.custom.as_ref()) {
    ...
    (None, Some(_)) => anyhow::bail!("no Registry agent is selected; pass an ID to `loops agents setup <id> --yes`"),
    (None, None) => anyhow::bail!("no Registry agent is selected; run `loops agents select <id>` or pass an ID to `loops agents setup <id> --yes`"),
  }
}
...
async fn doctor(cwd: &Path) -> Result<bool> {
  ...
  if !path.exists() {
    ...
    println!(
      "Next: run `loops init`, then `loops agents select <id>` or configure [agent.custom]."
    );
    return Ok(false);
  }

  let config = match loops_domain::config::read_config(cwd).await {
    Ok(config) => config,
    Err(error) => {
  ...
  match (
    ...
    (None, None) => {
      ...
      println!("ACP preflight: NOT READY");
      println!("Next: run `loops agents select <id>` or configure [agent.custom].");
      Ok(false)
    }
    (Some(id), None) => doctor_registry_agent(cwd, id).await,
    (None, Some(custom)) => {
      println!("configured source: custom ACP command");
      println!("install provenance: loops.toml [agent.custom]");
...
fn print_preferences(preferences: &loops_domain::config::AgentPreferences) {
  let configured = |preference: &loops_domain::config::RolePreference| {
...
async fn doctor_registry_agent(cwd: &Path, id: &str) -> Result<bool> {
  ...
  let resolved = match RegistryClient::default().resolve(&cache, id).await {
    ...
    Err(error) if binary_installation_required(&error) => {
      ...
      println!("ACP preflight: NOT READY");
      println!("Next: run `loops agents setup {id} --yes`, then rerun `loops agents doctor`.");
      return Ok(false);
    }
    Err(error) => {
...
async fn doctor_acp(launch: AcpLaunch) -> Result<bool> {
  match launch.readiness().await {
    ...
    Err(error) => {
      ...
      println!(
        "Next: fix the ACP agent launch or authentication setup, then rerun `loops agents doctor`."
      );
      Ok(false)
    }
  }
...
async fn login(cwd: &Path, requested_method: Option<&str>) -> Result<()> {
  ...
  let Some(method_id) = requested_method else {
    if readiness.auth_methods.is_empty() {
      ...
      println!("Next: run `loops agents login <method-id>` for an agent-owned method.");
    }
    return Ok(());
  };
  ...
  if method.mechanism != AcpAuthMechanism::Agent {
    anyhow::bail!("authentication method {method_id:?} requires {}; Loops does not handle credentials or interactive terminal authentication. Follow the agent's documented authentication flow.", method.mechanism.label());
  }

  launch.login(method_id).await?;
  println!("Authentication request accepted by the agent.");
  println!("Next: complete any agent-owned authentication flow, then rerun `loops agents doctor`.");
  Ok(())
}
...
async fn configured_acp_launch(cwd: &Path) -> Result<AcpLaunch> {
  ...
  match (
    ...
    (None, None) => anyhow::bail!(
      "no ACP launch source configured; run `loops agents select <id>` or configure [agent.custom]"
    ),
    ...
    (Some(id), None) => {
      let resolved = RegistryClient::default()
        ...
        .map_err(|error| {
          anyhow::anyhow!(
            "could not resolve Registry agent {id:?}; run `loops agents doctor`: {error:#}"
          )
        })?;
...
fn print_command_preflight(command: &str) -> bool {
  if command_available(command) {
    ...
    println!("Next: install the configured source, then rerun `loops agents doctor`.");
    false
  }
}
