...
use clap::Parser;
use loops_acp::acp::AcpRuntime;
use loops_domain::model::RunStatus;
use loops_runtime::{backend::AgentBackend, controller::manual_verify, store};
use loops_tui::tui;
...
impl App {
  ...
  async fn initialize(&self) -> Result<()> {
    let config = loops_domain::config::ensure_config(&self.cwd).await?;
  ...
  fn print_initialization(&self) {
    println!("Initialized loops in {}", self.cwd.display());
    if std::io::stdout().is_terminal() {
      println!("Run `loops agents` to browse ACP Registry agents, then `loops agents select <id>`");
    } else {
      println!("Set exactly one of agent.id (Registry) or [agent.custom] in loops.toml; no agent was selected automatically");
    }
  }
