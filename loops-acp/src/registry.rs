...
pub use loops_runtime::backend::LaunchMetadata;
...
impl Default for RegistryClient {
  fn default() -> Self {
    Self {
      source: env::var("LOOPS_REGISTRY_URL").unwrap_or_else(|_| DEFAULT_INDEX_URL.into()),
    }
  }
}
...
fn validate_index(index: &RegistryIndex) -> Result<()> {
  if !index.version.starts_with("1.") {
    anyhow::bail!(
      "unsupported Registry index version {:?}; next action: use a Registry v1 source or update Loops",
      index.version
    )
  }
