...
5. Obtain the proposal shape from `tenet_contract_schema`, then submit the complete typed proposal with `tenet_contract_propose`.
6. On `pending_approval`, present the exact proposal ID and digest; every requirement and obligation ID and statement; every primary verifier and authority mapping; and every oracle-assurance ID, criterion, verifier, and authority mapping.
7. Ask whether the human explicitly approves that exact ID and digest. Never self-approve or infer approval from silence, continuation, generic acknowledgement, or an earlier proposal.
