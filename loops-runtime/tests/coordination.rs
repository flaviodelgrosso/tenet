...
impl TempRepo {
  fn new() -> Self {
    let path = std::env::temp_dir().join(format!("loops-test-{}", Uuid::new_v4()));
    std::fs::create_dir_all(&path).expect("create temp repository");
    run_git(&path, &["init"]);
    run_git(&path, &["config", "user.name", "Loops Test"]);
    run_git(&path, &["config", "user.email", "loops-test@localhost"]);
    std::fs::write(path.join(".gitignore"), ".loops/\n").expect("write gitignore");
...
impl AgentBackend for FakeBackend {
  ...
  async fn implement(
    ...
    if matches!(self.mode, BackendMode::ProtectedMutation) {
      fs::write(ctx.cwd.join("loops.toml"), "modified").await?;
    }
...
async fn configured_controller(
  ...
  fs::write(
    repository.path().join("loops.toml"),
    toml::to_string_pretty(&config).expect("serialize config"),
  )
  .await
  .expect("write config");
  fs::create_dir_all(repository.path().join(".loops"))
    .await
    .expect("create loops dir");
  fs::write(repository.path().join(".loops/spec.md"), "diamond")
    .await
    .expect("write spec");
  run_git(repository.path(), &["add", "loops.toml"]);
...
async fn cancellation_after_partial_completion_cleans_all_worktrees() {
  ...
  assert_eq!(state.status, loops_domain::model::RunStatus::Stopped);
