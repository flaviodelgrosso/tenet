...
impl Controller {
        ...
        for cycle in 1..=ctx.config.max_cycles {
                ...
                && reconciliation.next_work_unit.is_none()
            {
                ...
                let report = self
                    ...
                    .await?;
                if !report.passed {
                    let reason = state.blocked_reason.clone().unwrap_or_else(|| {
                        "Final deterministic verification did not converge".into()
                    });
                    ...
                    && assessment.next_work_unit.is_none()
                {
                    ...
                    state.current_work_unit = None;
                    state.last_summary =
                        "All requirements have evidence and deterministic gates pass".into();
                    self.publish(&ctx.events, state).await?;
            ...
            if !report.passed {
                let reason = state.blocked_reason.clone().unwrap_or_else(|| {
                    format!("Verification did not converge for {}", work_unit.id)
                });
                return self.block(ctx, state, &reason).await;
            }
    ...
    async fn verify_with_repairs(
        ...
        let _ =
            store::save_evidence(&self.cwd, &format!("verify-{label}-attempt-0"), &report).await?;
        if report.passed {
...
fn validate_reconcile(catalog: &RequirementCatalog, result: &ReconcileResult) -> Result<()> {
    ...
    if result.complete && (!all_satisfied(catalog, result) || result.next_work_unit.is_some()) {
        bail!("reconciliation claimed complete without every requirement satisfied and nextWorkUnit=null");
    }
...
pub async fn manual_verify(cwd: &Path) -> Result<VerificationReport> {
    ...
    let _ =
        store::save_evidence(cwd, &format!("manual-{}", Utc::now().timestamp()), &report).await?;
    Ok(report)
}
