...
5. Once the verification policy can support the material obligations, derive the completion contract from the specification and verification plan. Get the proposal shape with `tenet contract schema --json`, submit it with `tenet contract propose --file <path> --json`, and proceed to explicit operator approval.
...
7. Only after the user explicitly approves the exact proposal identified in the confirmation question, run `tenet contract approve --proposal <id> --digest <digest> --json`. Never self-approve or infer approval from silence, continuation, a generic acknowledgement unrelated to the exact confirmation, or a previous approval.
8. Approval is bound to the exact proposal ID and digest that were shown to the user. If the proposal content, specification, policy, ID, or digest changes after the approval question was presented—or the CLI rejects the approval as stale or mismatched—discard that approval, show the current proposal, and request a new explicit Yes/No approval. Never reuse an earlier approval for a changed proposal.
9. After contract admission and before product implementation begins, freeze the complete authority surface in an immutable Git commit containing the admitted specification, verification policy, completion contract, and any authority-owned oracle assets.
   The resulting commit is only a proposed authority revision; Tenet or the coding agent must not silently select it as A.
