...
use crate::{
    ...
    events::RunEvent,
    model::{
        ArchitectOutput, CompletedWorkUnit, ReconcileResult, RequirementCatalog,
...
impl AgentBackend for OmpRpcBackend {
    async fn architect(&self, ctx: &BackendContext, spec: &str) -> Result<ArchitectOutput> {
        ...
        self.run_typed(ctx, WorkerRole::Architect, &prompt, architect_schema())
            .await
    }
    ...
    async fn reconcile(
        ...
        self.run_typed(ctx, WorkerRole::Reconcile, &prompt, reconcile_schema())
            .await
    }
    ...
    async fn implement(
        ...
        self.run_typed(ctx, WorkerRole::Implement, &prompt, worker_summary_schema())
            .await
    }
    ...
    async fn repair(
        ...
        self.run_typed(ctx, WorkerRole::Repair, &prompt, worker_summary_schema())
            .await
    }
    ...
    async fn assess(
        ...
        self.run_typed(ctx, WorkerRole::Assess, &prompt, reconcile_schema())
            .await
    }
}
...
impl RpcProcess {
    ...
    async fn initialize_protocol(&mut self) -> Result<()> {
        ...
        let supports_v2 = ready
            ...
            .unwrap_or(false);
        if supports_v2 {
            self.send(&json!({"id":"protocol-v2","type":"negotiate_protocol","protocolVersion":2}))
                .await?;
    ...
    async fn answer_headless_ui(&mut self, frame: &Value) -> Result<()> {
        ...
        match method {
            "confirm" => {
                self.send(&json!({"type":"extension_ui_response","id":id,"confirmed":false}))
                    .await?
            }
            "select" | "input" | "editor" | "cancel" | "open_url" => {
                self.send(&json!({"type":"extension_ui_response","id":id,"cancelled":true}))
                    .await?
            }
            _ => {}
        }
