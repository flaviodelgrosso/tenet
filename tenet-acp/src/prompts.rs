...
- Use `artifact/project_verification` only for repository-wide build, lint, or suite obligations. Use `artifact/named_project_check` with an exact controller-provided configured check name for behavior covered by that check. Source inspection is supporting/advisory only and cannot be selected as an authoritative proof contract until the controller provides a mechanical source predicate. Use `artifact/executable_evidence` only when a trusted verifier is configured; otherwise it will be rejected. Use `human_attestation` for subjective obligations. Compose requirements with `all` or `any` only where every branch has those semantics.
